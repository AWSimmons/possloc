...
%General todo- when do I want a figure, when do I want inline equation*?
\begin{document}
...
%Need to add:  What does it meant to be quantumly accessible? What's the complexity of finding out if one is? define "context"
%use consistent notation for the measurement setting selection

%Mention how such tables can otherwise appear eg Abramsky- otherwise explain why in letter to referee.

%Put in Hardy's original table- move material from locality decision problem to this background sec
...
%We, then, need only to answer the question of the necessary and sufficient nature of \textsc{2-Robust}ness; we note that choosing a specific 1 to complete to a grid represents therefore an assignment of one variable, and the structure of the table then might result in the forced selection of another variable's assignment, if there is a 0 in the same subcolumn as the selected 1. Therefore by the same logic as before, the \textsc{(2,3)-PossLoc} decision problem is \textbf{NP}-complete. %Rewrite this paragraph to be more helpful.
\end{proof}
...
%if it hits a point, then choose some point on the great circle that was OK
%stick an axis through that point and rotate the GC by epsilon
%if THAT hits a point, rotate it back eps/2
%continue until you have a good hyperplane
%each point that you hit must be totally unique so you can only hit all n of the points in the worst case so it's O(n)
%... i'm not sure i want this in the paper it's too stupid but a reviewer has asked me to justify why i claimed that this bit of an algorithm was ok in linear time
%reword this theorem maybe
\end{thm}
\begin{proof}
Each measurement row corresponds to a two-outcome measurement. Without loss of generality, we can take these to be projective measurements since any two-outcome POVM is a convex mixture of such projectors. Since the possibility table would then be a statistical mixture of two or more other tables, noting that the statistical mixture of two local tables is always local, if there exists a quantum mechanically accessible table using POVMs for measurement choices for Alice, there must exist one using PVMs. If our entangled state is pure, then this means that each outcome steers Bob's system to one of two pure states that convexly mix to Bob's reduced state.
...
In general, however, increasing the quantum dimension of the underlying system can lead to a strict increase in the number of available tables as long as $d<\min(l^2,m^2)$. That there is no advantage to increasing $d>\min(l^2,m^2)$ can be seen by considering a Schmidt decomposition of the underlying state, and that it strictly increases the number of available tables can be seen by the fact that a subtable with only $k$ 1 entries, in a table in which no measurement subrow or subcolumn consists entirely of zeroes, is only possible when the quantum dimension $d$ satsifies $d\geq lm -k+1$. %This is getting a bit definition-dependent.. consider rewording or even removing.
\begin{figure}
\begin{equation*} 
...
%Any triple-outcome measurement for Bob will be enacted as a triple of projectors which include the identity operator in their convex hull. The two-outcome measurement is chosen such that the projector for the measurement subrow with the desired 0 steers the state into the one \emph{orthogonal} to the projector assigned to relevant measurement subcolumn. This is a different choice of representation than the one used earlier, but will be adopted now for diagrammatic clarity. Explicitly, our embedding has to conform to the following geometric restrictions:
...
\item An impossible event happens when the point corresponding to that measurement subcolumn and the point corresponding to that measurment subrow are the same point. %This happens due to our new choice of state representation in our illustrations; the projector for a measurement subrow steers the state into the one orthogonal to the point that it is demarcated on the Bloch equator. The POVM element is proportional to the projector onto that point on the equator, and so when the two are represented by the same point, they are orthogonal which corresponds to an impossible event.
\end{itemize}

A specific quantum scenario will now be explored alongside a characterisation of some of the problem instances that can be embedded into it. Given a \textsc{3-SAT} instance, let us assign each variable a colour: blue or red. Clauses with variables of certain colourings and negation status will be realisable with this construction. For example, as can be seen in Figure \ref{Basicdiagram}, a clause consisting of a red positive literal, a red negative literal, and a blue negative literal, \emph{e.g.} $(\color{red}{l_1} \color{black}{\wedge} \color{red}{\neg l_2} \color{black}{\wedge} \color{blue}{\neg l_3})$, forms a valid POVM. 
...
\draw [fill=ffqqqq] (3.296294172570832,-2.2659313157895093) circle (3pt);
\draw [fill=ffqqqq] (-1.3609931181462014,3.7613425438742856) circle (3pt);
\draw [fill=ffqqqq] (-0.5438388074049572,3.962857473031342) circle (3pt);
\draw [fill=ffqqqq] (1.5730879602280303,-3.6776887129535054) circle (3pt);
\draw [fill=ffqqqq] (-0.5438388074049572,3.962857473031342) circle (3pt);
\draw [fill=qqqqff] (-3.3,-2.27) circle (3pt);
\draw [fill=qqqqff] (-1.57,-3.68) circle (3pt);
\draw [fill=qqqqff] (0.54,3.96) circle (3pt);
\draw [fill=qqqqff] (1.36,3.76) circle (3pt);
\draw [fill=uuuuuu] (0.,-4.) circle (3pt);
\draw [fill=uuuuuu] (0.,4.) circle (3pt);
